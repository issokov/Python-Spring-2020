...
class SudokuSolver:
        ...
        while not all_filled:
            ...
            for x, y in product(range(9), repeat=2):
                if variants[x][y] and len(variants[x][y]) == 1:
                    ...
                    has_loners = True
                    all_filled = False
                    self.history += str(self.hist_counter) + '. '
                    self.hist_counter += 1
                    self.history += "Один возможный вариант: [{}, {}]" \
                                    " = {}\n".format(x+1, y+1, field[x][y])
        return has_loners
                    ...
                    if count == 1:
                        for y in range(9):
                            if sudoku.variants[x][y]\
                                ...
                                self.history += str(self.hist_counter) + '. '
                                self.hist_counter += 1
                                self.history += "В строке можно поставить" \
                                                " только сюда: [{}, {}]" \
                                                " = {}\n".\
                                    format(x+1, y+1, sudoku[(x, y)])
                                has_single = True
                                all_filled = False
                    ...
                    if count == 1:
                        for y in range(9):
                            if sudoku.variants[y][x]\
                                ...
                                self.history += str(self.hist_counter) + '. '
                                self.hist_counter += 1
                                self.history += "В столбце можно поставить" \
                                                " только сюда:" \
                                                " [{}, {}] = {}\n"\
                                    .format(y+1, x+1, sudoku[(y, x)])
        ###
        for x, y, in product(range(0, 7, 3), repeat=2):
            sqr = defaultdict(lambda: 0)
            for i, j in product(range(3), repeat=2):
                if sudoku.variants[x + i][y + j]:
                    for v in sudoku.variants[x + i][y + j]:
                        sqr[v] += 1
            for value, candidate in sqr.items():
                if candidate == 1:
                    for i, j in product(range(3), repeat=2):
                        if sudoku.variants[x + i][y + j] is not None and\
                                value in sudoku.variants[x + i][y + j]:
                            sudoku[(x + i, y + j)] = value
                            self.history += str(self.hist_counter) + '. '
                            self.hist_counter += 1
                            self.history += "В квадрате значение '{}'" \
                                            " можно поставить " \
                                            "только сюда: [{}, {}]\n".\
                                format(sudoku[(y, x)], y+1, x+1)
        ###
        return has_single
    ...
    def try_random_step(self, sudoku=None):
        ...
        self.history += str(self.hist_counter) + '. '
        self.hist_counter += 1
        self.history += "Нет точных вариантов для [{}, {}]," \
                        " кандидаты: {}, выбираем:  = {}\n"\
            .format(*key, candidates, sudoku[key])
        return True
